...
from tkcalendar import DateEntry  # Asegúrate de tener instalada la librería tkcalendar
...
class RegistroVentas:
    def __init__(self, root):
        ...
        self.correo_usuario = "trabajoh060904@gmail.com"
        self.contrasena_usuario = "qscq dbxk urnq sbev"
        ...
        # Filtros de fecha
        self.frame_filtro = tk.Frame(root, bg="peach puff")
        self.frame_filtro.pack(pady=(10, 20))

        self.label_fecha_inicio = tk.Label(self.frame_filtro, text="Fecha Inicio:")
        self.label_fecha_inicio.grid(row=0, column=0)
        self.fecha_inicio = DateEntry(self.frame_filtro, width=12)
        self.fecha_inicio.grid(row=0, column=1)

        self.label_fecha_fin = tk.Label(self.frame_filtro, text="Fecha Fin:")
        self.label_fecha_fin.grid(row=0, column=2)
        self.fecha_fin = DateEntry(self.frame_filtro, width=12)
        self.fecha_fin.grid(row=0, column=3)

        btn_filtrar = tk.Button(self.frame_filtro, text="Filtrar Ventas", command=self.filtrar_ventas)
        btn_filtrar.grid(row=0, column=4)
    ...
    def mostrar_ventas(self):
        ...
        for v in ventas:
            try:
                ...
                self.tree.insert("", "end", values=(v[0], v[1], direccion, productos_resumen, total_formatted, fecha_venta))
            except Exception as e:
                print(f"Error procesando venta {v[0]}: {e}")

    def filtrar_ventas(self):
        """Filtra las ventas según las fechas ingresadas."""
        ...
        for v in ventas:
            try:
                fecha_venta = v[5]  # Suponiendo que la fecha está en la posición 5
    ...
    def generar_pdf(self, direccion, venta_id, cliente, productos, total, fecha):
        ...
        pdf.cell(190, 10, txt="Detalles de los Productos:", ln=True)
        pdf.ln(5)
        ...
        for producto in productos.split(','):
            ...
            if len(partes) == 2:
                producto_id = partes[0].strip()  # Obtener el ID del producto
                nombre = partes[1].strip()
                cantidad_y_precio = partes[1].strip().split('(') if len(partes) == 2 else []
                if len(cantidad_y_precio) == 2:
                    try:
                        ...
                        pdf.cell(30, 10, txt=producto_id[:10], border=1, align="C")  # Limitar ID a 10 caracteres
                        pdf.cell(60, 10, txt=nombre[:30], border=1)  # Limitar nombre a 30 caracteres
                        pdf.cell(25, 10, txt=f"{cantidad:.2f}", border=1, align="C")
                        pdf.cell(35, 10, txt=f"${precio_unitario:.2f}", border=1, align="C")
                        pdf.cell(40, 10, txt=f"${precio_total:.2f}", border=1, align="C")
                        pdf.ln()
                    except ValueError:
                        pdf.cell(190, 10, txt=f"Error en formato para el producto: {producto}", ln=True)
                else:
                    pdf.cell(190, 10, txt=f"Formato inválido para el producto: {producto}", ln=True)

        # Total final
        pdf.ln(5)
        pdf.set_font("Arial", size=12, style="B")
        pdf.cell(150, 10, txt="Total:", align='R', border=0)
